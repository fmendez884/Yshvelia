...
public class HitboxSpawner : EditorWindow
{
    private string objectBaseName = "Hitbox";
    ...
    [MenuItem("Tools/Hitbox Spawner")]
    public static void ShowWindow()
    {
        GetWindow<HitboxSpawner>("Hitbox Spawner");
    }

    private void OnGUI()
    {
        GUILayout.Label("Hitbox Spawner", EditorStyles.boldLabel);
        ...
        {
            GUILayout.Label("No Hitboxes", EditorStyles.boldLabel);
        }

        ready = spherePrefab != null && capsulePrefab != null && characterPrefab != null;

        if (GUILayout.Button("Spawn Hitboxes") && ready)
        {
            SpawnHitboxes();
        ...
        {
            GUILayout.Label("Please assign all prefabs and create hitbox container.", EditorStyles.boldLabel);
        }
    ...
    private void AdjustHitboxes()
    {
        if (hitboxes != null)
        {
            foreach (GameObject hitbox in hitboxes)
            {
                ...
                if (hitbox.TryGetComponent(out CapsuleCollider capsuleCollider))
                {
                    capsuleCollider.radius = (objectScale * capsuleScale) / 2;
                    capsuleCollider.height = capsuleHeight;
                }

                if (hitbox.TryGetComponent(out SphereCollider sphereCollider))
                {
    ...
    private void ClearHitboxesInScene()
    {
        if (hitboxes != null)
        {
            ...
            {
                if (gameObject.name.Contains("Hitbox"))
                {
    ...
    private void CreateHeadSphere(BoneItem boneItem, Dictionary<string, int> boneNameCounts, BoneItem neckBone = null)
    {
        ...
        string hitboxName = GenerateUniqueHitboxName(boneItem.BoneObject.name, boneNameCounts);
        GameObject newObject = Instantiate(spherePrefab, headCenter, boneItem.BoneObject.rotation);
        newObject.name = hitboxName;
        ...
        // newObject.transform.SetParent(hitboxContainer.transform);
        newObject.transform.SetParent(boneItem.BoneObject);
        hitboxes.Add(newObject);
    ...
    {
        string hitboxName = GenerateUniqueHitboxName(parentBone.BoneObject.name, boneNameCounts);
        GameObject newObject = Instantiate(capsulePrefab);
        newObject.name = hitboxName;
        CapsuleCollider collider = newObject.GetComponent<CapsuleCollider>();
    ...
    private string GenerateUniqueHitboxName(string boneName, Dictionary<string, int> boneNameCounts)
    {
